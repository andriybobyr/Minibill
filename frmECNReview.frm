...
Begin VB.Form frmECNReview 
   BackColor       =   &H00C0E0FF&
   Caption         =   "Minbill - ECN Review"
   ClientHeight    =   6015
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6555
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGECNInfo 
      ...
      Columns(2).Caption=   "Models Included"
      Columns(2).DataField=   ""
      Columns(2)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
      Columns(3)._VlistStyle=   0
      Columns(3)._MaxComboItems=   5
      Columns(3).Caption=   "Parts Included"
      Columns(3).DataField=   ""
   ...
   End
End
Attribute VB_Name = "frmECNReview"
...
Private Sub cmdDisplayModels_Click()
    With frmModelsByECN
        .lblECN.Caption = TDBGECNInfo.Columns(0).Value
        .ShowModels
        .Show vbModal
    End With
End Sub
...
Private Sub Form_Load()
    ...
    With rsData
        ...
        .Source = "select case when start_ecn_number = ' ' then quit_ecn_number else start_ecn_number end as ecn_number, " & _
            "case when max(start_ecn_date) = ' ' then case when max(quit_ecn_date) = ' ' then ' ' else substring(max(quit_ecn_date),5,2) + '/' + " & _
            "substring(max(quit_ecn_date),7,2) + '/' + substring(max(quit_ecn_date),1,4) end " & _
            "else substring(max(start_ecn_date),5,2) + '/' + " & _
            "substring(max(start_ecn_date),7,2) + '/' + substring(max(start_ecn_date),1,4)  end as effective_date, " & _
            "count(distinct v_mnb_model_part.model_number) as models_covered, count(distinct part_id) as parts_covered " & _
            ...
            "where start_ecn_number <> ' ' or quit_ecn_number <> ' ' " & _
            "group by case when start_ecn_number = ' ' then quit_ecn_number else start_ecn_number end " & _

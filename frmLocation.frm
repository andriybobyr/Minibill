...
Begin VB.Form frmLocation 
   Caption         =   "MiniBill - Stocking Location Maintenance"
   ClientHeight    =   4110
   ClientLeft      =   60
   ClientTop       =   630
   ClientWidth     =   7710
...
Private Sub chkObsolete_Click()
    ...
    If blnObsolete = mrsDatabase!stocking_location_obsolete_flag Then
        Exit Sub
    End If
    
    If mrsDatabase!stocking_location_obsolete_flag <> blnObsolete Then
        mblnRecChanged = True
        mrsDatabase!stocking_location_obsolete_flag = chkObsolete.Value
        ...
        If blnObsolete Then
            ...
            mrsDatabase!stocking_location_obsolete_date = _
                CDate(mskObsoleteDate.Text)
        Else
            mskObsoleteDate.Text = "__/__/____"
            mrsDatabase!stocking_location_obsolete_date = Null
        End If
    End If
...
Private Sub cmdObsoleteDateCalendar_Click()
    ...
    If Not IsNull(dlgCalendar.mdteSelectedDate) Then
        ...
        If mrsDatabase!stocking_location_obsolete_date <> _
...
Private Sub Form_Load()
    ...
    If mrsDatabase.RecordCount > 1 Then
        frmLocationDisplay.Show vbModal
    End If
...
Private Sub mrsDatabase_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
    ...
    With mrsDatabase
        If Not .EOF And Not .BOF Then
            ...
            txtLocationID.Text = !stocking_location_id
            txtLocationDescription.Text = Trim(!stocking_location_description)
            If !stocking_location_obsolete_flag Then
                chkObsolete.Value = 1
                mskObsoleteDate.Text = _
                    Format$(!stocking_location_obsolete_date, "mm/dd/yyyy")
            Else
                mskObsoleteDate.Text = "__/__/____"
                chkObsolete.Value = 0
            End If
...
Sub RetrieveData()
    ...
    With mrsDatabase
        ...
        .Source = "v_prod_stocking_location_all"
    ...
    If mrsDatabase.EOF Then
        mrsDatabase.AddNew
        txtLocationID.Text = vbNullString
        txtLocationDescription.Text = vbNullString
        chkObsolete.Value = 0
        mskObsoleteDate.Text = "__/__/____"
        mskObsoleteDate.Enabled = False
...
Private Sub txtLocationDescription_GotFocus()
    ' Purpose:  Select the field for easy update
    
    txtLocationDescription.SelStart = 0
    txtLocationDescription.SelLength = Len(txtLocationDescription.Text)
End Sub

Private Sub txtLocationDescription_KeyPress(KeyAscii As Integer)
    mblnRecChanged = True
End Sub
...
Private Sub txtLocationDescription_Validate(Cancel As Boolean)
    ...
    If Len(Trim(txtLocationDescription.Text)) = 0 Then
        Cancel = True
        MsgBox "Location Description Is Required!", _
            vbExclamation + vbOKOnly, _
            "Location Description Validation"
        GoTo PROC_EXIT
Else
        mrsDatabase!stocking_location_description = _
...
PROC_ERR:
    Call ShowError(Me.Name, "txtLocationDescription_Validate", _
        Err.Number, Err.Description)
    GoTo PROC_EXIT
End Sub
...
Private Sub txtLocationID_GotFocus()
    ...
    If Len(txtLocationID.Text) > 0 Then
...
Private Sub txtLocationID_Validate(Cancel As Boolean)
    ...
    Else
        ...
        ' in the recordset.
        If Trim(txtLocationID.Text) <> Trim(mrsDatabase!stocking_location_id) Then
            Set rsDuplicate = mrsDatabase.Clone
            rsDuplicate.Find ( _
                "stocking_Location_ID = '" & txtLocationID.Text & "'")
            If rsDuplicate.EOF Then
                mrsDatabase!stocking_location_id = txtLocationID.Text
                Cancel = False
            Else
                MsgBox "Location ID " & txtLocationID.Text & _
                    " already exists."
                Cancel = True
            End If
...
PROC_ERR:
    Call ShowError(Me.Name, "txtLocationID_Validate", _
        Err.Number, Err.Description)
    GoTo PROC_EXIT
End Sub
...
Function ValidEntries() As Boolean
    ...
    Call txtLocationID_Validate(blnCancel)
    ...
    Call txtLocationDescription_Validate(blnCancel)
    ...
    For Each fld In mrsDatabase.Fields
        If fld.OriginalValue <> fld.Value Then
            mblnRecChanged = True
            mrsDatabase!stocking_location_last_updated = Now()
            mrsDatabase!stocking_location_updated_by = gclsSQLServer.UserID
            Exit For
        End If
    Next fld
